...
var ARROW, LOCATION;
var ACTIVE_VECTOR_COLOR = '#fceffc';
...
var vectorsNone = function() {
  ...
  $("div.option").find('span').css('color', '#fceffc');
...
var vectorsTerrestres = function() {
  toggleVectorsInCategory('terrestre', '#f29100');
  updateBoxValues(updatedBoxValues());
};

var vectorsNavieros = function() {
  toggleVectorsInCategory('naviero', '#f3d400');
  updateBoxValues(updatedBoxValues());
};
...
var toggle24Hours = function() {
  ...
  if (ENABLED_24_HOURS) {
    $("#24-hours").css('color', '#d8e400');
    vectorsNone();
    pageInit(DATA_24H);
  } else {
    $("#24-hours").css('color', '#fceffc');
    vectorsNone();
    pageInit(DATA);
  }
};
...
var toggleVectors = function(client, div, color) {
  if ($(div).hasClass("active-option")) {
    ...
    $(div).find('span').css('color', '#fceffc');
  } else {
...
var updateVectorGroup = function(group, obj, color) {
...
var insertClientOptions = function() {
  ...
  for (var client in CLIENTS) {
    if (CLIENTS.hasOwnProperty(client)) {
      if (CLIENTS[client].category === 'naviero') {
        selector = '#naviero-clients';
      } else if (CLIENTS[client].category === 'terrestre') {
        selector = '#terrestre-clients';
      } else {
        console.error(
          `Unexpected client.category for ` +
          `${client} (${CLIENTS[client].category})`
        );
      }
      $(selector).append(clientOptionHTML(client));
    }
  }
};
